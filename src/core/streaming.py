...
class StreamMetrics:
    """Collect metrics for streaming operations"""
    ...
    def record_stream_success(self, chunks: int, bytes_sent: int):
        """Record successful stream completion"""
        self.successful_streams += 1
        self.total_chunks += chunks
        self.total_bytes += bytes_sent
        
    def record_stream_failure(self):
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self.successful_streams / self.total_streams 
                if self.total_streams > 0 else 0
            ),
            "total_chunks": self.total_chunks,
            "total_bytes": self.total_bytes,
            "average_chunks_per_stream": (
                self.total_chunks / self.successful_streams
                if self.successful_streams > 0 else 0
            )
        }